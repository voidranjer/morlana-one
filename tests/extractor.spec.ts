...
import { ChequingApiResponse, ScenePlusApiResponse } from "./lib/scotiabank/types";
...
undetectedTest("scotiabank (chequing)", async ({ page, request }) => {
  ...
  const testDone = new Promise<void>((resolve) => {
    page.on('response', async (response) => {
      ...
      if (match) {
        expect(response.ok()).toBeTruthy();
        const body: ChequingApiResponse = await response.json();
...
undetectedTest("scotiabank (scene+)", async ({ page, request }) => {
  undetectedTest.setTimeout(300_000); // 5 minutes
  ...
  expect(process.env.SCOTIA_SCENEPLUS_URL).toBeDefined();
  await page.goto(process.env.SCOTIA_SCENEPLUS_URL!);
  ...
  const testDone = new Promise<void>((resolve) => {
    page.on('response', async (response) => {
      ...
      // Always on the 17th for some reason.
      const match = response.url().match(/transactions[^?]*\?fromDate=(\d{4}-\d{2}-\d{2})&toDate=(\d{4}-\d{2}-\d{2})/);

      if (match) {
        expect(response.ok()).toBeTruthy();
        const body: ScenePlusApiResponse = await response.json();

        console.log(`Received ${body.data.settled.length} transactions for: ${match[1]} to ${match[2]}`);

        // Log response body from bank to a file
        await logToFile(body);

        // Post transactions to Firefly
        const transactions: TransactionPayload[] = body.data.settled
          .map(t => {
            const isWithdrawal = t.transactionType === "DEBIT"
            const type = isWithdrawal ? "withdrawal" : "deposit";
            const amount = t.transactionAmount.amount;
            ...
            const external_id = t.key;
            const source_name = isWithdrawal ? process.env.SCENEPLUS_FIREFLY_ACCOUNT_NAME : undefined;
            const destination_name = isWithdrawal ? undefined : process.env.SCENEPLUS_FIREFLY_ACCOUNT_NAME;
